...
def floor_div(args):
    ...
    mid = 1
    while devide != nil:
        if isinstance(devide.first, Pair):
            mid *= calc_eval(devide.first)
            devide = devide.rest
        else:
            mid *= devide.first
            devide = devide.rest
...
def eval_and(expressions):
    ...
    nxt = expressions.rest
    if ret == False and isinstance(ret, bool):
        return ret
    while nxt != nil:
        if isinstance(nxt.first, Pair):
            ret = calc_eval(nxt.first)
            nxt = nxt.rest
        else:
            ret = nxt.first
            nxt = nxt.rest
